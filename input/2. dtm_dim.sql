...
with
    villes as (
        select distinct
            ville_nom nom, ville_region region, ville_pays pays, ville_zone "zone"
        from stg.stg_commande
    )
select
...
with
    double_ville as (
        select nom, count(1) nb from dtm.dim_ville group by 1 having count(1) > 1
    )
select
    row_number() over (order by "zone", pays, region, nom) id_ville,
    md5(concat("zone", pays, region, nom)) cd_ville,
...
with
    ...
    marque_double as (
        select 'Office Star' marque
        union all
        select 'Binney & Smith' marque
        union all
